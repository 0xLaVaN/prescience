...
export default function MissionControlClient({ initialData }) {
  // Start with SSR-provided data — no loading spinner on first render
  const [data,     setData]     = useState(initialData || null);
  const [loading,  setLoading]  = useState(false);   // false: data already available
  ...
  const load = useCallback((background = false) => {
    ...
    fetch(`/data/mission-control.json?t=${Date.now()}`)
      .then(r => r.json())
      .then(d => { setData(d); setLastSync(new Date()); if (!background) setLoading(false); })
      .catch(e => { setError(e.message); if (!background) setLoading(false); });
  }, []);

  // Background refresh on mount (picks up any newer deploy data)
  useEffect(() => { load(true); }, [load]);

  // Full refresh when user clicks refresh or interval fires
  useEffect(() => { if (refresh > 0) load(false); }, [load, refresh]);
    ...
    <>
      ...
      <div style={{ minHeight:'100vh', background:'#0a0a0f', color:'#fff', paddingTop:80 }}>
        ...
        <div style={{ position:'relative', zIndex:10, maxWidth:1100, margin:'0 auto', padding:'0 24px' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:32 }}>
            <div>
              ...
              <div style={{ fontSize:12, color:'rgba(255,255,255,.3)', fontFamily:'monospace', marginTop:6 }}>
                {lastSync ? `Refreshed ${fmtRelative(lastSync.toISOString())}` : (genAt ? `Data from ${fmtRelative(genAt.toISOString())}` : '')}{' '}
                {dataAge !== null && (
                  <span style={{ color: dataAge > 30 ? '#f0a000' : 'rgba(255,255,255,.3)' }}>
                    · Snapshot {dataAge === 0 ? 'fresh' : `${dataAge}m old`}
                  </span>
                )}
              </div>
            </div>
          ...
          {/* Stats row — populated immediately from initialData */}
          ...
          {error && !data && (
            <div style={{ padding:40, textAlign:'center', color:'#ff3366',
              fontFamily:'monospace', fontSize:13, background:'rgba(255,51,102,.05)',
              border:'1px solid rgba(255,51,102,.15)', borderRadius:12, marginBottom:20 }}>
          ...
          <div style={{ background:'rgba(255,255,255,.02)', border:'1px solid rgba(255,255,255,.06)',
            ...
            <div style={{ display:'grid', gridTemplateColumns:'24px 1fr 90px 80px 90px 90px',
              ...
              <div/><div>Job</div><div>Status</div>
            ...
            {loading ? (
              /* Only shows during manual refresh, not on first load */
          ...
          <div style={{ textAlign:'center', fontFamily:'monospace', fontSize:11,
            ...
            Snapshot synced every 6h · Auto-refresh every 2min
            {genAt && ` · Last sync: ${genAt.toUTCString().slice(0,22)}`}
          </div>
        </div>
      </div>
    </>
